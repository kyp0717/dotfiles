...
| `pi/skills/` + `pi/extensions/` | pi's skill set (30 pstack-strict skills, plus `graft` with a pending pi-conversion note) and extensions (`unslop.ts`). Stowed to `~/.pi/agent/`: `stow -d ~/dotfiles/harness -t ~/.pi/agent pi`. |
